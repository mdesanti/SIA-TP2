...
files.each do |file|
	noninformed.each do |method|
		...
		output.flush
		output.puts `java -Xmx2g -jar TP1-SIA-0.0.1-SNAPSHOT-jar-with-dependencies.jar --checksymmetry true --cachedepth 36 --filename #{file} --method #{method} --timeoutseconds 120 --onlyresults true`
		output.flush
	end
end
	...
	heuristics.each do |heuristic|
		costfunction.each do |costfunction|
			noninformed.each do |method|
				...
				output.flush
				output.puts `java -Xmx2g -jar TP1-SIA-0.0.1-SNAPSHOT-jar-with-dependencies.jar --checksymmetry true --cachedepth 36 --filename #{file} --method #{method} --timeoutseconds 120 --onlyresults true --costfunction #{costfunction} --heuristic #{heuristic}`
				output.flush			
			end
		end
	end
